...
const en = {
  ...
  configure: {
    ...
    title: 'Your home looks lived-in. Even when it isn’t.',
    lede: 'KEVIN 3 comes in Grey with the Grey front cover included. Add more colours if you like, then check out — free shipping across the EU and 30-day returns.',
    ...
    covers_heading: 'Change Kevin’s look any time',
    covers_note: '{{ price }} each · any colour, any quantity',
    covers_included: 'Your Kevin ships with the Grey cover. Add extra colours to match your room.',
    cover_soon: 'Soon',
    ...
    usp_2_body: 'No cameras, no microphones. Every simulation runs on the device itself — never streamed, never shared.',
    usp_3_title: 'Protection that runs itself',
    usp_3_body: 'Set weekly schedules or let geofencing switch Kevin on when you leave. 70+ hours of simulations, never the same loop twice.',
...
nl.configure = {
  ...
  title: 'Je huis lijkt bewoond. Ook als het dat niet is.',
  lede: 'KEVIN 3 komt in Grey, met de grijze frontcover inbegrepen. Voeg desgewenst extra kleuren toe en reken af — gratis verzending in de hele EU en 30 dagen retour.',
  ...
  covers_heading: 'Verander de look van Kevin wanneer je wilt',
  covers_note: '{{ price }} per stuk · elke kleur, elk aantal',
  covers_included: 'Je Kevin wordt geleverd met de grijze cover. Voeg extra kleuren toe die bij je kamer passen.',
  ...
  gallery_slide_status: 'Afbeelding {{ n }} van {{ total }}',
  badge_free_shipping: 'Gratis verzending in de hele EU',
  usps_label: 'Waarom Kevin',
  usp_1_title: 'Inbrekers kiezen een ander huis',
  usp_1_body: 'Kevin wisselt het licht, laat bewegende schaduwen zien en speelt alledaagse geluiden, zodat er vanaf de straat duidelijk iemand thuis is.',
  usp_2_title: 'Niets kijkt of luistert mee',
  usp_2_body: 'Geen camera’s, geen microfoons. Elke simulatie draait op het apparaat zelf — nooit gestreamd, nooit gedeeld.',
  usp_3_title: 'Bescherming die zichzelf regelt',
  usp_3_body: 'Stel weekschema’s in of laat geofencing Kevin inschakelen zodra je vertrekt. 70+ uur simulaties, nooit twee keer dezelfde loop.',
...
// ---------------------------------------------------------------------------
{
  ...
  lines.push('-%}');
  // `render` isolates variables, so the table is injected into seo-hreflang.liquid itself.
  const hreflangPath = path.join(root, 'snippets/seo-hreflang.liquid');
  const START = '{%- comment -%}ROUTES:START (generated by scripts/build-locales.mjs){%- endcomment -%}';
  const END = '{%- comment -%}ROUTES:END{%- endcomment -%}';
  const src = fs.readFileSync(hreflangPath, 'utf8');
  const a = src.indexOf(START), b = src.indexOf(END);
  if (a < 0 || b < 0) throw new Error('seo-hreflang.liquid is missing ROUTES:START/END markers');
  const table = lines.slice(1).join('\n'); // drop the standalone comment line
