...
if [[ "$DRY_RUN" -eq 0 ]] && ! higgsfield account status >/dev/null 2>&1; then
  echo "Auth required:"
...
run_photoshoot() {
  ...
  url="$(higgsfield product-photoshoot create \
    ...
    --image "$ref" \
    --aspect_ratio "$aspect" \
    --count 1 2>&1 | extract_url)"
  if [[ -z "$url" ]]; then
    echo "  ERROR: no URL returned for $outfile" >&2
    return 1
  fi
...
run_marketplace_asset() {
  ...
  url="$(higgsfield marketplace-cards create \
    ...
    --brand_context "Mitipi Kevin, grey heather fabric wedge, Swiss engineered, minimal white palette" 2>&1 | extract_url)"
  if [[ -z "$url" ]]; then
    echo "  ERROR: no URL returned for $outfile" >&2
    return 1
  fi
...
if [[ "$RUN_HERO" -eq 1 ]]; then
  ...
  run_photoshoot "kevin-hero-protecting-children.jpg" lifestyle_scene 3:4 "$LIFESTYLE_REF" \
  ...
  run_photoshoot "kevin-hero-seniors-widows.jpg" lifestyle_scene 3:4 "$LIFESTYLE_REF" \
  ...
  run_photoshoot "kevin-hero-luxury-dutch-villa.jpg" lifestyle_scene 3:4 "$LIFESTYLE_REF" \
  ...
  run_photoshoot "kevin-hero-students-university.jpg" lifestyle_scene 3:4 "$LIFESTYLE_REF" \
  ...
  run_photoshoot "kevin-hero-dutch-houseboat.jpg" lifestyle_scene 3:4 "$LIFESTYLE_REF" \
...
if [[ "$RUN_FEATURES" -eq 1 ]]; then
  echo ""
  echo "=== Problem tiles ==="
  run_photoshoot "kevin-feature-problem-alarms.jpg" conceptual_product 4:3 "$LIFESTYLE_REF" \
  ...
  run_photoshoot "kevin-feature-problem-cameras.jpg" conceptual_product 4:3 "$LIFESTYLE_REF" \
    "CCTV camera watching dark empty living room, footage does not make home feel occupied, cold monitor glow, editorial"
  run_photoshoot "kevin-feature-problem-lights.jpg" conceptual_product 4:3 "$LIFESTYLE_REF" \
  ...
  echo ""
  echo "=== Personas ==="
  run_photoshoot "kevin-persona-travel.jpg" lifestyle_scene 4:3 "$LIFESTYLE_REF" \
    "Kevin on windowsill, packed suitcase nearby, Dutch suburban home dusk, travel away mode, product 25% frame width"
  run_photoshoot "kevin-persona-alarm-complement.jpg" lifestyle_scene 4:3 "$LIFESTYLE_REF" \
    "Kevin beside disarmed alarm keypad, German townhouse entry hall, complementary deterrence, product 25% frame width"
  run_photoshoot "kevin-persona-living-alone.jpg" lifestyle_scene 4:3 "$LIFESTYLE_REF" \
  ...
  run_photoshoot "kevin-feature-schedules.jpg" lifestyle_scene 16:9 "$LIFESTYLE_REF" \
    "Kevin in living room, phone on table showing schedule blur, evening weekly routine, product 25% frame width"
  run_photoshoot "kevin-feature-geo.jpg" lifestyle_scene 16:9 "$LIFESTYLE_REF" \
  ...
  run_photoshoot "kevin-feature-multi-device.jpg" lifestyle_scene 16:9 "$LIFESTYLE_REF" \
