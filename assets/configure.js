(function () {
  ...
  function renderTotal() {
    ...
    if (els.perDevice) {
      if (state.plan === 'buy' && state.quantity > 1 && variant) {
        ...
        els.perDevice.textContent = window.LurafiCart.formatMoney(variant.price) + ' per device';
      } else {
  ...
  function renderSummary() {
    ...
    if (els.summaryColor && variant) {
      els.summaryColor.textContent = variant.color;
    }
    if (els.summaryPlan) {
      els.summaryPlan.textContent = state.plan === 'subscribe' ? 'Kevin+ monthly' : 'One-time purchase';
    }
  }
  ...
  function renderImage() {
    ...
    if (els.image.src !== src) {
      ...
      setTimeout(function () {
        els.image.src = src;
        els.image.alt = 'Kevin in ' + variant.color;
        els.image.style.opacity = '1';
      }, 150);
    }
    if (els.colorName) els.colorName.textContent = variant.color;
  }
    ...
    cta.addEventListener('click', function () {
      ...
      if (!variant) {
        if (els.error) {
          ...
          els.error.textContent = 'Please assign products in Theme settings → Products.';
        }
        return;
      }
